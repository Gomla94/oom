                                ...
                                <div class="card-box table-responsive">
                                    <table id="datatable-fixed-header" class="table table-striped table-bordered" style="width:100%">
                                        <thead>
                                            <tr>
                                                <th>الترتيب</th>
                                                <th>{{ Helper::admin_trans('title') }}</th>
                                                <th>{{ Helper::admin_trans('image') }}</th>
                                                <th>{{ Helper::admin_trans('comments') }}</th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr style="font-weight: bold;">
                                                ...
                                                <td>{{ $item->title }}</td>
                                                <td style="width: 20%;">
                                                    <img src="{{ Helper::storage_image($item->image) }}" style="width: 100%;" alt="{{ $item->title }}">
                                                </td>
                                                <td><a style="color:green; bolder" href="{{ route('admin.' . $route . '.comments.index', [$element => $item->id]) }}">{{ $item->comments()->count() . ' تعليق' }}</a></td>
                                               @if(isset($item->group->grade->title)) <td> {{$item->group->grade->title}}</td>@else <td></td>  @endif
                                               @if(isset($item->group->title)) <td> {{$item->group->title}}</td>@else <td></td>  @endif
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        ...
                                                        {!! Form::close() !!}
                                                        <button type="button" class="btn btn-danger" onclick="if(confirm('إذا قمت بمسح الدرس سوف يتم مسح جميع اختبارات الطلبة المتعلقة بهذا الدرس')){ $('#item-{{ $item->id }}').submit(); }"><i class="fa fa-trash"></i></button>
                                                    </div>
                                                </td>
                                            </tr>
                                            @empty
