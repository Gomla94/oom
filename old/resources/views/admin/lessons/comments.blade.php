                                    ...
                                    <table id="datatable-fixed-header" class="table table-striped table-bordered" style="width:100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>{{ Helper::admin_trans('replies') }}</th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{ $item->id }}</td>
                                                <td>{{ $item->user }}</td>
                                                <td>{{ App\Models\Comment::where('parent_id', $item->id)->get()->count() }}</td>
                                                <td>
                                                    <div class="btn-group">
                                                        <a href="{{ route('admin.lessons.' . $route . '.single_comment', ['lesson' => $item->lesson->id, $element => $item->id]) }}" class="btn btn-success"><i class="fa fa-share"></i></a>
                                                        {!! Form::open(
                                                        [
                                                            'route'     => ['admin.lessons.' . $route . '.destroy', $item->lesson->id, $item->id],
                                                            'method'    => 'DELETE',
                                                            'id'        => 'item-' . $item->id
                                                        ]) !!}
