...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
       ...
       // if( !Auth::attempt($login) || ! Auth::user()->status)
        ...
        $device     = Auth::user()->devices()->where('mac_address', $maco)->first();
        ...
        if(is_null($device))
        {
            ...
            Auth::logout();
            return back()->withErrors( 'هذا الجهاز غير مسجل لدينا , برجاء الرجوع للإدراة لتفعيل الجهاز');
        ...
        {
            Auth::logout();
            return back()->withErrors( 'لا يمكنك تسجيل الدخول من هذا الجهاز , يمكنك الرجوع للإدارة لتفعيل الجهاز');
        }
