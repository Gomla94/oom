                                ...
                                <div class="card-box table-responsive">
                                    <table id="datatable-fixed-header" class="table table-striped table-bordered" style="width:100%">
                                        <thead>
                                            <tr>
                                                <th>الترتـيب</th>
                                                <th>{{ Helper::admin_trans('name') }}</th>
                                                <th>{{ Helper::admin_trans('roles') }}</th>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @foreach ($item->roles as $role)
                                                        <div class="span badge-dark" style="width: auto;">{{ $role->name }}</div>
                                                    @endforeach
                                                </td>
                                                <td>
                                                    <div class="btn-group">
                                                        ...
                                                        <button type="button"  @if($item->id == 6) disabled @endif class="btn btn-danger" onclick="if(confirm('{{ Helper::admin_trans('sure') }}')){ $('#item-{{ $item->id }}').submit(); }"><i class="fa fa-trash"></i></button>
                                                    </div>
                                                </td>
                                            </tr>
                                            @empty
                                                <th colspan="4" class="text-center">{{ Helper::admin_trans('empty', ['el' => Helper::admin_trans($elements)]) }}</th>
                                            @endforelse
                                        </tbody>
                                    </table>
                                </div>
