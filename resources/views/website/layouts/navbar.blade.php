									...
									<ul class="dropdown-menu-list small-slimscroll-style" data-handle-color="#637283">
										...
										<li>
											<a href="#">
												...
												<span class="subject">
													<span class="from"> @if($message->model == 'App\Models\Student') {{ auth()->user()->name }}  @else الإدارة @endif </span>
													<span class="time"> {{ Carbon\Carbon::parse($message->created_at)->diffForHumans()}} </span>
												</span>
												<span class="message" style="margin-right: 50px"> {{ $message->details }}</span>
											</a>
										</li>
										@endforeach
								        @endif
